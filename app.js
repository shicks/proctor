...
function playAlarmSequence() {
    ...
    alarmInterval = setInterval(() => {
        ...
        count++;
        if (count > 10) clearInterval(alarmInterval);
    }, 1000);
...
function updateControls(active) {
    ...
    // When timer is active, button is "STOP"
    if (active) {
    ...
    // When timer is inactive, button is "RESET"
    else {
        el.stop.textContent = 'RESET';
        el.stop.disabled = false; // Always enabled when inactive
        el.stop.className = "w-full bg-gray-700 hover:bg-gray-600 text-gray-300 font-bold py-4 rounded-xl text-lg shadow-lg active:scale-95 transition";
    }
}
